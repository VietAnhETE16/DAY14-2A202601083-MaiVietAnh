...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        is_relevant: list[int] = []
        for chunk in contexts:
            chunk_tokens = _tokenize(chunk)
            cov = len(chunk_tokens & expected_tokens) / len(expected_tokens)
            is_relevant.append(1 if cov >= relevance_threshold else 0)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric: {rubric}\n"
            "Please score each criterion on a scale of 0.0 to 1.0 and provide reasoning."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if isinstance(item_scores, dict):
                all_scores.extend(float(v) for v in item_scores.values() if isinstance(v, (int, float)))
        ...
        if len(scores_batch) > 1:
            ...
            positional_bias = bool(first_avg > other_avg + 0.05)
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict[str, Any]:
        ...
        new_avg_faithfulness = sum(r.faithfulness for r in new_results) / new_n if new_n > 0 else 0.0
        new_avg_relevance = sum(r.relevance for r in new_results) / new_n if new_n > 0 else 0.0
        new_avg_completeness = sum(r.completeness for r in new_results) / new_n if new_n > 0 else 0.0

        baseline_avg_faithfulness = sum(r.faithfulness for r in baseline_results) / base_n if base_n > 0 else 0.0
        baseline_avg_relevance = sum(r.relevance for r in baseline_results) / base_n if base_n > 0 else 0.0
        baseline_avg_completeness = sum(r.completeness for r in baseline_results) / base_n if base_n > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures, start=1):
            ...
            if suggestions:
                ...
                fix = "Review pipeline and improve context/prompt"
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0) > 0:
            ...
            suggestions.append("Refine system prompt to strictly instruct LLM to only answer based on retrieved context")

        if categories.get("irrelevant", 0) > 0 or categories.get("off_topic", 0) > 0:
            suggestions.append("Add Query Rewriting and Intent Detection steps to improve question-answer relevance")
            suggestions.append("Add few-shot examples demonstrating concise and direct answers to user queries")

        if categories.get("incomplete", 0) > 0:
            suggestions.append("Increase chunk size or top_k in RAG pipeline to reduce context fragmentation and improve recall")
            suggestions.append("Add prompt instructions requiring step-by-step checklist coverage for comprehensive responses")

        default_suggestions = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Deploy a reranker (Cross-Encoder) to boost Context Precision and place relevant chunks at top ranks",
        ]
